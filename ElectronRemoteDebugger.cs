...
namespace DiscordConnect
{
    class ElectronRemoteDebugger
    {
        ...
        public List<Tuple<Uri, string>> windows()
        {
            ...
            var resp = this.requests_get($"http://{host}:{port}/json/list?t={secondsSinceEpoch}");
            if (resp is JArray)
            {
        ...
        private async Task Send(WebSocket webSocket, string expression)
        {
        ...
        private async Task<string> sendrcv(Uri webSocketUri, string expression)
        {
        ...
        private JArray requests_get(string url, int tries=5, int delay=1)
        {
